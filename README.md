...
If `localhost` networking is blocked on your machine you can build a completely
offline version instead. Use `start-static.bat` which builds static files and opens `out/index.html` directly. The build step now copies assets into each page directory so navigation works when opened from the file system.
